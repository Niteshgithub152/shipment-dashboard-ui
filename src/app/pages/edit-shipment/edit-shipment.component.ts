...
ngOnInit(): void {
  this.shipmentIds = this.route.snapshot.paramMap.get('id');
  console.log('Shipment ID:', this.shipmentIds);

  if (this.shipmentIds) {
    this.httpService.getShipmentbyShipmentId(this.shipmentIds).subscribe({
      next: (response: any) => {
        ...
        this.editShipmentForm = this.fb.group({
          ...
          lattitude: [shipment.trackingDetails?.lattitude || ''],
          longitude: [shipment.trackingDetails?.longitude || ''],
        });
      }
    });
  }
